                    ...
                    try {
                        if (event == SMSSDK.EVENT_GET_VERIFICATION_CODE) {
                            ...
                            if (result == SMSSDK.RESULT_COMPLETE) {
                                ...
                                loginByPhoneTask.execute(Constant.URL_LOCAL + "LoginByPhonenum");
                            } else {
                                // TODO 处理错误的结果
    ...
    public class LoginByPhoneTask extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            switch (userStatus.getStatus()) {
                //手机号未注册
                case "2":
                    Toast.makeText(LoginByPhoneActivity.this, "手机号未注册，请返回注册！", Toast.LENGTH_SHORT).show();
                    break;
                //登录成功，写入用户信息
                case "0":
                    Log.e("--------", "手机号登录");
                    SharedPreferences.Editor editor = sharedPreferences.edit();
                    editor.putString("user", userStatus.getUser().toString());
                    intent = new Intent(LoginByPhoneActivity.this, TabHostActivity.class);
                    startActivity(intent);
                    break;
                //登录失败
                case "1":
